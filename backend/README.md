...
Интеграционные тесты лежат в отдельной папке:

- `tests/integration`

Они поднимают временную SQLite-базу, применяют миграции, загружают seed и проверяют:
...
- отмену записи
- обновление профиля
- работу HTTP API поверх реальной БД
...
```bash
go test ./tests/integration -run TestHTTPAPIAppointmentAndProfileFlow -v
```
